...
int I2cBus::Begin()
{
	if(_bus_st == I2cBusStatus::BUS_UNINIT)
	{
...
/****************************************************************
 * Check availability.
 * To prevent overlapping data read (someone request a read
 * before previous read gets the data out), the status 
 * I2cBusStatus::BUS_ASK_FINISH, which represents the data is ready
 * for read, is considered busy. One must call FetchRegReadData()
 * to reset the status.
 *****************************************************************/
bool I2cBus::IsAvailable()
{
	return (_bus_st == I2cBusStatus::BUS_STDBY) ||
		   ( _bus_st == I2cBusStatus::BUS_LAST_FAILED);
}
...
int I2cBus::ReadRegBlocked(uint8_t dev_addr, const uint8_t reg, 
						   unsigned int len, uint8_t* bfr, 
						   unsigned long t_out_ms)
{
	unsigned long t_start = millis();
	unsigned long now;
	int ret = ReadReg(dev_addr, reg, len);
	if( ret )  return ret; 		// something went wrong
	do {
		ret = UpdateReadReg();
		now = millis();
